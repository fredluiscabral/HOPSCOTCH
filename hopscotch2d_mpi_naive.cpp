...
// - Troca bloqueante de halo norte/sul após cada sub-fase (4 trocas por passo)
// - Mantém a lógica numérica "naive" original
//
// Compilar:
//   mpicxx -O3 -std=c++17 -DOMPI_SKIP_MPICXX=1 -DMPICH_SKIP_MPICXX=1 \
//          hopscotch2d_mpi_naive_1d.cpp -o hopscotch2d_mpi_naive_1d
...
static inline size_t idx2(int i, int j, int ld) {
    return static_cast<size_t>(i) * static_cast<size_t>(ld) + static_cast<size_t>(j);
}
...
static void exchange_halo_rows(std::vector<double>& A, int ni, int ld,
                               int nbr_north, int nbr_south, MPI_Comm comm)
{
    MPI_Status status;
    // Norte: envia linha 1, recebe em halo 0
    if (nbr_north != MPI_PROC_NULL) {
        MPI_Sendrecv(&A[idx2(1,    0, ld)], ld, MPI_DOUBLE, nbr_north, 10,
                     &A[idx2(0,    0, ld)], ld, MPI_DOUBLE, nbr_north, 11,
                     comm, &status);
    }
    // Sul: envia linha ni, recebe em halo ni+1
...
int main(int argc, char** argv) {
    ...
    const int ld = N; // largura completa, incluindo fronteiras j=0 e j=N-1
    const size_t NN_local = static_cast<size_t>(ni + 2) * static_cast<size_t>(ld);
    ...
    exchange_halo_rows(U_new, ni, ld, nbr_north, nbr_south, MPI_COMM_WORLD);
    ...
    for (int step = 0; step < T; ++step) {
        ...
        exchange_halo_rows(U_old, ni, ld, nbr_north, nbr_south, MPI_COMM_WORLD);
