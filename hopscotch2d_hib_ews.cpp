// hopscotch2d_hib_ews.cpp
// Equação do calor 2D (Hopscotch) — versão HÍBRIDA MPI+OpenMP (EWS: explicit work-sharing)
// - Decomposição 2D por MPI (cartesiana PxQ), halo de 1 célula em cada lado
// - Dentro de cada rank: OpenMP com divisão EXPLÍCITA do domínio (faixas contíguas em i)
// - Sincronização GLOBAL por fase com `#pragma omp barrier` (sem sincronização local vizinho-a-vizinho)
// - Trocas de halo após cada sub-fase (4 trocas por passo) para manter dependências entre ranks
//
...
//
// Compilar (exemplo):
//   mpicxx -O3 -fopenmp -DOMPI_SKIP_MPICXX=1 -DMPICH_SKIP_MPICXX=1 \
//          hopscotch2d_hib_ews.cpp -o hopscotch2d_hib_ews
//
// Executar (exemplo OpenMPI):
//   mpirun -np 4 --map-by ppr:1:socket:PE=48 --bind-to core \
...
// Divisão 1D equilibrada: total itens em `parts` partes. Retorna start (0-based) e count.
...
int main(int argc, char** argv) {
    ...
    // Recomendado quando há OpenMP (mesmo com MPI em thread única / funneled)
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        if (world_rank == 0) {
            std::cerr << "Erro: MPI não forneceu suporte a threads MPI_THREAD_FUNNELED.\n";
        }
    ...
    // Imprime threads por rank (cada rank imprime uma linha)
    #pragma omp parallel
    {
        #pragma omp single
        {
    ...
    // Datatype de coluna (halo vertical) para o array com stride L
    MPI_Datatype COL_T;
               ...
               nbr_north,nbr_south,nbr_west,nbr_east,COL_T,L)
    {
        ...
        // Divisão explícita das linhas locais [1..ni] entre threads
        int istart0=0, icount=0;
        split_range(ni, nt, tid, istart0, icount); // 0-based em [0..ni-1]
        const int iLocal = 1 + istart0;
        const int fLocal = iLocal + icount - 1;

        for (int step=0; step<T; ++step){

            // ===== Fase 1 (explícita): escreve U_old a partir de U_new em (ig+jg+m) par =====
            if (icount > 0) {
            ...
            // ===== Fase 2 (semi-implícita): escreve U_old em (ig+jg+m) ímpar =====
            if (icount > 0) {
                for (int ii=iLocal; ii<=fLocal; ii+=TILE){
                    const int i_end = std::min(fLocal, ii+TILE-1);
                    for (int jj=1; jj<=nj; jj+=TILE){
            ...
            // ===== Fase 3 (explícita): escreve U_new a partir de U_old em (ig+jg+m) par =====
            if (icount > 0) {
                for (int ii=iLocal; ii<=fLocal; ii+=TILE){
                    const int i_end = std::min(fLocal, ii+TILE-1);
                    for (int jj=1; jj<=nj; jj+=TILE){
            ...
            // ===== Fase 4 (semi-implícita): escreve U_new em (ig+jg+m) ímpar =====
            if (icount > 0) {
                for (int ii=iLocal; ii<=fLocal; ii+=TILE){
                    const int i_end = std::min(fLocal, ii+TILE-1);
                    for (int jj=1; jj<=nj; jj+=TILE){
