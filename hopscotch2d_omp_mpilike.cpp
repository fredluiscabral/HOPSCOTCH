...
static_assert(sizeof(PaddedAtomicInt) == CACHELINE_BYTES, "PaddedAtomicInt must occupy one cache line");
...
static inline void ensure_unew_halos(ThreadDomain& dom, int tid, int nt, int epoch) noexcept {
    const int slot = epoch & 1;
    const std::size_t ld = dom.ld;
    if (tid > 0) {
        wait_until_at_least(dom.unew_top_ready.value, epoch);
        copy_row(dom.u_new.data() + L(0, 0, ld), dom.halo_unew_top[slot].data(), ld);
    }
    if (tid + 1 < nt) {
        wait_until_at_least(dom.unew_bottom_ready.value, epoch);
        copy_row(dom.u_new.data() + L(dom.local_n + 1, 0, ld), dom.halo_unew_bottom[slot].data(), ld);
    }
}

static inline void ensure_uold_halos(ThreadDomain& dom, int tid, int nt, int epoch) noexcept {
    const int slot = epoch & 1;
    const std::size_t ld = dom.ld;
    if (tid > 0) {
        wait_until_at_least(dom.uold_top_ready.value, epoch);
        copy_row(dom.u_old.data() + L(0, 0, ld), dom.halo_uold_top[slot].data(), ld);
    }
    if (tid + 1 < nt) {
        wait_until_at_least(dom.uold_bottom_ready.value, epoch);
        copy_row(dom.u_old.data() + L(dom.local_n + 1, 0, ld), dom.halo_uold_bottom[slot].data(), ld);
    }
}
...
int main() {
    ...
    if (!load_params_strict("param.txt", N, alpha, T, TILE)) {
        std::cerr << "Erro: não foi possível abrir param.txt (obrigatório).\n";
        return 1;
    }
    ...
    const std::size_t ld = round_up(static_cast<std::size_t>(N), CACHELINE_BYTES / sizeof(double));
    ...
    for (int tid = 0; tid < num_threads; ++tid) {
        ...
        const std::size_t main_elems = static_cast<std::size_t>(local_n + 2) * ld;
        ...
        dom.unew_top_ready.value.store(tid == 0 ? 0 : -1, std::memory_order_relaxed);
        dom.unew_bottom_ready.value.store(tid == num_threads - 1 ? 0 : -1, std::memory_order_relaxed);
        dom.uold_top_ready.value.store(tid == 0 ? 0 : -1, std::memory_order_relaxed);
        dom.uold_bottom_ready.value.store(tid == num_threads - 1 ? 0 : -1, std::memory_order_relaxed);
    ...
    #pragma omp parallel num_threads(num_threads) shared(doms, N, h, D, x0, y0)
    {
        ...
        for (int il = 0; il <= dom.local_n + 1; ++il) {
            std::fill_n(dom.u_new.data() + static_cast<std::size_t>(il) * ld_local, ld_local, 0.0);
            std::fill_n(dom.u_old.data() + static_cast<std::size_t>(il) * ld_local, ld_local, 0.0);
        }
        for (int k = 0; k < 2; ++k) {
        ...
        for (int il = 1; il <= dom.local_n; ++il) {
            ...
            for (int j = 1; j <= N - 2; ++j) {
                const double y = j * h;
                row_new[j] = std::exp(-D * ((x - x0) * (x - x0) + (y - y0) * (y - y0)));
                row_old[j] = 0.0;
            }
        }
        ...
        if (tid == num_threads - 1) {
            std::fill_n(dom.u_new.data() + L(dom.local_n + 1, 0, ld_local), ld_local, 0.0);
            std::fill_n(dom.u_old.data() + L(dom.local_n + 1, 0, ld_local), ld_local, 0.0);
        }
    }
    ...
    #pragma omp parallel num_threads(num_threads) shared(doms, num_threads, N, T, TILE, lam, denom)
    {
        ...
        for (int step = 0; step < T; ++step) {
            ...
            // Fase 1: interior primeiro
            for (int ii = 1; ii <= dom.local_n; ii += TILE) {
                const int i_end = std::min(dom.local_n, ii + TILE - 1);
                const bool touch_top = (ii == 1);
                const bool touch_bottom = (i_end == dom.local_n);
                if (touch_top || touch_bottom) continue;
                for (int jj = 1; jj <= N - 2; jj += TILE) {
                    const int j_end = std::min(N - 2, jj + TILE - 1);
                    for (int i = ii; i <= i_end; ++i) {
                        const int ig = dom.start_g + (i - 1);
                        double* row_old = dom.u_old.data() + L(i, 0, ld_local);
                        const double* row_new = dom.u_new.data() + L(i, 0, ld_local);
                        const double* row_new_up = dom.u_new.data() + L(i - 1, 0, ld_local);
                        const double* row_new_dn = dom.u_new.data() + L(i + 1, 0, ld_local);
                        for (int j = jj; j <= j_end; ++j) {
                            if (((ig + j + m0) & 1) == 0) {
                                row_old[j] = row_new[j] + lam * (row_new_dn[j] + row_new_up[j] + row_new[j + 1] + row_new[j - 1] - 4.0 * row_new[j]);
                            } else {
                                row_old[j] = row_new[j];
                            }
                        }
            ...
            for (int pass = 0; pass < 2; ++pass) {
                const bool want_top = (pass == 0);
                const int ii = want_top ? 1 : (1 + ((dom.local_n - 1) / TILE) * TILE);
                if (ii < 1 || ii > dom.local_n) continue;
                const int i_end = std::min(dom.local_n, ii + TILE - 1);
                const bool touch_top = (ii == 1);
                const bool touch_bottom = (i_end == dom.local_n);
                if (want_top && !touch_top) continue;
                if (!want_top && (!touch_bottom || touch_top)) continue;
                for (int jj = 1; jj <= N - 2; jj += TILE) {
                    const int j_end = std::min(N - 2, jj + TILE - 1);
                    for (int i = ii; i <= i_end; ++i) {
                        const int ig = dom.start_g + (i - 1);
            ...
            for (int ii = 1; ii <= dom.local_n; ii += TILE) {
                ...
                for (int jj = 1; jj <= N - 2; jj += TILE) {
                    ...
                    for (int i = ii; i <= i_end; ++i) {
                        ...
                        double* row_old = dom.u_old.data() + L(i, 0, ld_local);
                        const double* row_new = dom.u_new.data() + L(i, 0, ld_local);
                        const double* row_old_up = dom.u_old.data() + L(i - 1, 0, ld_local);
                        const double* row_old_dn = dom.u_old.data() + L(i + 1, 0, ld_local);
                        for (int j = jj; j <= j_end; ++j) {
                            if (((ig + j + m0) & 1) == 1) {
                                row_old[j] = (row_new[j] + lam * (row_old_dn[j] + row_old_up[j] + row_old[j + 1] + row_old[j - 1])) / denom;
                            }
                        }
                    }
                }
            ...
            for (int ii = 1; ii <= dom.local_n; ii += TILE) {
                ...
                for (int jj = 1; jj <= N - 2; jj += TILE) {
                    ...
                    for (int i = ii; i <= i_end; ++i) {
                        const int ig = dom.start_g + (i - 1);
                        double* row_new = dom.u_new.data() + L(i, 0, ld_local);
                        const double* row_old = dom.u_old.data() + L(i, 0, ld_local);
                        const double* row_old_up = dom.u_old.data() + L(i - 1, 0, ld_local);
                        const double* row_old_dn = dom.u_old.data() + L(i + 1, 0, ld_local);
                        for (int j = jj; j <= j_end; ++j) {
                            if (((ig + j + m1) & 1) == 0) {
                                row_new[j] = row_old[j] + lam * (row_old_dn[j] + row_old_up[j] + row_old[j + 1] + row_old[j - 1] - 4.0 * row_old[j]);
                            } else {
                                row_new[j] = row_old[j];
                            }
                        }
            ...
            for (int ii = 1; ii <= dom.local_n; ii += TILE) {
                ...
                for (int jj = 1; jj <= N - 2; jj += TILE) {
                    ...
                    for (int i = ii; i <= i_end; ++i) {
                        ...
                        for (int j = jj; j <= j_end; ++j) {
                            if (((ig + j + m1) & 1) == 1) {
                                row_new[j] = (row_old[j] + lam * (row_new_dn[j] + row_new_up[j] + row_new[j + 1] + row_new[j - 1])) / denom;
                            }
                        }
                    }
                }
