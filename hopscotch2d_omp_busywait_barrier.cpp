// hopscotch2d_omp_busywait_barrier.cpp
// Equação do calor 2D (Hopscotch) — OpenMP com divisão em faixas e espera ocupada local,
// + barreira global APENAS ao final de cada passo.
// param.txt é OBRIGATÓRIO.
// Saída: output.txt (amostrado a cada 16 pontos) + tempo do laço principal em stdout.
...
int main() {
    ...
    // Vetor de progresso (um contador por thread) — alocado fora para evitar cópias
    std::atomic<int>* progress = nullptr;

    // Paraleliza por faixas (linhas) com sincronização local; barreira global ao fim do passo
    #pragma omp parallel default(shared) shared(progress)
    {
        const int tid = omp_get_thread_num();
        const int nt  = omp_get_num_threads();

        // Alocação do vetor de progresso apenas uma vez
        #pragma omp single
        {
            progress = new std::atomic<int>[nt];
            for (int k=0; k<nt; ++k) progress[k].store(0, std::memory_order_relaxed);
        ...
        int fLocal = iLocal + H - 1;
        if (tid < R) fLocal += 1;

        // Função de espera: garante que vizinhos tenham atingido "expected"
        ...
        int m_local = 0; // paridade local (mantida consistente via handshakes)
        ...
        for (int step=0; step<T; ++step) {
            ...
            // ====== FASE 3 (explícita) ======
            // Espera vizinhos concluírem a Fase 2
            ...
            // Espera vizinhos concluírem a Fase 3
            ...
            // === Barreira global APENAS ao final do passo ===
            #pragma omp barrier
        } // step
    } // parallel
    ...
    // desaloca vetor de progresso (após região paralela)
    // (nota: progress foi declarado fora da região e é válido aqui)
    // Como não pode haver cópias, foi alocado com new[].
    // Se quiser evitar vazamento mesmo em exceções, troque por unique_ptr<atomic<int>[]>.
    // Aqui, como o programa termina depois, tanto faz, mas deixo correto:
    // (coloquei a deleção aqui; se mover para dentro, use single + barrier)
    // Reabre a leitura das variáveis? Não necessário; progress é visível aqui.
    // delete[] progress; // Descomente se quiser liberar antes do fim do programa.
    ...
    // Libera progress aqui, com segurança:
    // (se quiser medir só o laço, a liberação não impacta)
    // Nota: progress pode estar null se load_params falhou (mas saímos antes).
    // Como chegamos aqui, progress foi alocado.
    // Proteção adicional:
    // (Nada de threads ativas neste ponto.)
    // delete[] progress;  // opcional — programa está terminando.
