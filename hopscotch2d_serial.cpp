// hopscotch2d_serial.cpp
// Equação do calor 2D (Hopscotch) — versão serial
...
/*
    std::cout << "Parâmetros lidos de param.txt:\n"
              << "  N     = " << N << "\n"
              << "  alpha = " << alpha << "\n"
              << "  T     = " << T << " (nº de passos)\n"
              << "  TILE  = " << TILE << "\n"
                ...
                for (int i = ii; i <= i_end; ++i) {
                    for (int j = jj; j <= j_end; ++j) {
                        if ( ((i + j + m) & 1) == 0 ) {
                            ...
                            // Cópia para pontos não atualizados neste meio-passo
                            U_old[idx(i,j)] = U_new[idx(i,j)];
                        }
                    }
                }
